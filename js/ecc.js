...
function ECC_GFp_add_distinct(p2) {
    ...
    accumulated_output_info += cur_indent + "λ*(x1-x3) = " + temp2 + "\n";
    var y_res = ECC_GFp_p.sub(temp2, this.y);
    accumulated_output_info += cur_indent + "y3 = λ*(x1-x3)-y1 = " + y_res + "\n";
...
function ECC_GFp_double() {
    ...
    accumulated_output_info += cur_indent + "λ = (3*x1^2+a)/(2*y1) = " + lambda + "\n";
    ...
    accumulated_output_info += cur_indent + "x3 =λ^2-x1-x1 = " + x_res + "\n";
    ...
    accumulated_output_info += cur_indent + "λ*(x1-x3) = " + temp2 + "\n";
    var y_res = ECC_GFp_p.sub(temp2, this.y);
    accumulated_output_info += cur_indent + "y3 = λ*(x-x3)-x1 = " + y_res + "\n";
...
function ECC_GFp_add(p2) {
    ...
    var save_indent = cur_indent;
    cur_indent += "  ";
    accumulated_output_info += cur_indent + "Умножение " + this.format() + " на " + p2.format() + "\n";
    ...
    accumulated_output_info += cur_indent + "Сумма: " + add_result.format() + "\n";
    cur_indent = save_indent;
...
function ECC_GFp_invert() {
    ...
    accumulated_output_info += cur_indent + "  Обратное " + this.format() + " является " + inverse.format() + "\n";
...
function ECC_GFp_scalar_mul(n) {
    // indent one more level
    var save_indent = cur_indent;
    accumulated_output_info += cur_indent + "Скалярное произведение: " + n + "*" + this.format() + "\n";
    cur_indent += "  ";
...
//   }
    ...
    while (true) {
        ...
        {
            accumulated_output_info += cur_indent + "Подсчет " +
            ...
            accumulated_output_info += cur_indent + "Результат: " + res_mul +
        ...
        // double P
        accumulated_output_info += cur_indent + "Удовоение " +
            P2n_mul + "*" + this.format() + " = " + P2n.format() + "\n";
        P2n = P2n.ECC_GFp_double();             // get 2^(n+1) P (double it)
        P2n_mul *= 2;                 // update our multiplier
        accumulated_output_info += cur_indent + "Удовенное значение: " + P2n_mul + "*" +
            this.format() + " = " + P2n.format() + "\n";

        n = n >> 1;                   // halve n
    }

    accumulated_output_info += cur_indent + "Результат: " + res.format() + "\n";
    cur_indent = save_indent;
